...
PATCH="${progdir}/patch/13x/patch-usr-sbin-bsdinstall-distextract-distextract.c"
...
PATCH="${progdir}/patch/13x/patch-usr-sbin-bsdinstall-distfetch-distfetch.c"
patch --check -N < ${PATCH} > /dev/null 2>&1
[ $? -eq 0 ] && echo "apply ${PATCH}.." && patch < ${PATCH}
...
for i in adduser checksum docsinstall hardening hostname jail keymap mirrorselect mount netconfig netconfig_ipv4 netconfig_ipv6 rootpass services wlanconfig zfsboot bootconfig; do
	PATCH="${progdir}/patch/13x/patch-usr-sbin-bsdinstall-scripts-${i}"
	patch --check -N < ${PATCH} > /dev/null 2>&1
	[ $? -eq 0 ] && echo "apply ${PATCH}.." && patch < ${PATCH}
done
...
cp -a ${progdir}/patch/13x/auto ${SRC_ROOT}/usr.sbin/bsdinstall/scripts/auto
